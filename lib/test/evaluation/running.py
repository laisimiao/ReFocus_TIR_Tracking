...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    elif 'vtuavtir' in seq.dataset:
        base_results_path = os.path.join(tracker.results_dir, seq.name.split('/')[1])
    else:
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, num_gpu=8, epoch=60):
    ...
    def _results_exist():
        if seq.object_ids is None:
            if seq.dataset in ['trackingnet', 'got10k']:
                ...
                bbox_file = '{}/{}.txt'.format(tracker.results_dir, seq.name.split('/')[1])
            else:
...
def run_dataset(dataset, trackers, debug=False, threads=0, num_gpus=8, epoch=60):
